...
class medigate_cpp_gui_plugin_helps:
    ...
    MenuItem = collections.namedtuple(
        "MenuItem", ["action", "handler", "title", "tooltip", "shortcut", "popup"]
    )
...
class medigate_cpp_gui_plugin_t(idaapi.plugin_t, idaapi.UI_Hooks):
    ...
    def register_new_action(
        self, act_name, act_text, act_handler, shortcut, tooltip, icon
    ):
    ...
    def register_menu_actions(self):
        ...
        self.MENU_ITEMS.append(
            medigate_cpp_gui_plugin_helps.MenuItem(
                self.plugin_name + ":add_base_class",
                self.add_base_class,
                "add_base_class",
                "add base class",
                None,
        ...
        for item in self.MENU_ITEMS:
            if item.action == "-":
                continue
            self.register_new_action(
                item.action,
                item.title,
                medigate_cpp_gui_plugin_helps.IdaMenuActionHandler(self, item.action),
                item.shortcut,
                item.tooltip,
                -1,
            )
    ...
    def attach_main_menu_actions(self):
        for item in self.MENU_ITEMS:
            idaapi.attach_action_to_menu(
                "Edit/" + self.plugin_name + "/" + item.title,
                item.action,
                idaapi.SETMENU_APP,
            )
            ...
            for item in self.MENU_ITEMS:
                if item.popup:
                    idaapi.attach_action_to_popup(
                        widget, popup_handle, item.action, self.plugin_name + "/"
                    )
    ...
    def make_table(self, ClassName="B", VtableEa=0, VtableEaStop=0, Offset=0):
        ...
        if ok == 1:
            ...
            cpp_utils.make_vtable(
                class_name=class_name,
                vtable_ea=vtable_ea,
                vtable_ea_stop=vtable_ea_stop,
                offset_in_class=offset_in_class,
            )

    # add_baseclass(class_name, baseclass_name, baseclass_offset=0, to_refresh=False):
    def add_base_class(
        self, ClassName="C", BaseClassName="B", BaseClassOffset=0, Refresh=0
    ):
        ...
        if ok == 1:
            ...
            cpp_utils.add_baseclass(
                class_name=class_name,
                baseclass_name=baseclass_name,
                baseclass_offset=baseclass_offset,

...
def is_dummy_func_name(name):
    return name.startswith("nullsub_") or name.startswith("ret_") or ("_pure_virtual" in name) or ("_purecall" in name)


def can_rename_func(name):
    return not is_dummy_func_name(name)


def should_rename_member(name):
    return is_dummy_func_name(name)
...
def get_vtable_line(ea, stop_ea=None, ignore_list=None):
    ...
    is_pure_func = disasm.endswith("_pure_virtual") or disasm.endswith("_purecall")
    if not is_pure_func:
...
def post_func_name_change(new_name, ea):
    ...
    # Get data references, but filter out XREF_USER references
    ref_eas = []
    for xref in idautils.XrefsTo(ea, ida_xref.XREF_DATA):
        if idc.is_member_id(xref.frm) or not xref.user:
            ref_eas.append(xref.frm)
    ...
    for ref_ea in ref_eas:
        ...
        if not idc.is_member_id(ref_ea):
            ...
            # But check if there's a struct defined at ref_ea and if it's a vtable
            struct_tif = None
            try:
                flags = ida_bytes.get_flags(ref_ea)
                if ida_bytes.is_struct(flags):
                    # There's a struct applied, get its ID from opinfo
                    buf = ida_nalt.opinfo_t()
                    opi = ida_bytes.get_opinfo(buf, ref_ea, 0, flags)
                    if opi and opi.tid != BADADDR:
                        struct_tif = ida_typeinf.tinfo_t(tid=opi.tid)
            except Exception as e:
                logging.debug(f"post_func_name_change: Error checking struct at {hex(ref_ea)}: {e}")
            
            if struct_tif and struct_tif.is_struct() and is_struct_vtable(struct_tif):
                # It's a vtable struct, so this is a valid vtable reference
                logging.debug(f"post_func_name_change: Found vtable struct at {hex(ref_ea)}")
                continue
            
            # Not a vtable struct, so this is a non-vtable reference
        ...
        logging.debug(f"post_func_name_change: Member {hex(ref_ea)} is from a vtable struct (struct: {vtable_struct_name})")
    ...
    for sid in ref_eas:
        logging.debug(f"post_func_name_change: Processing loop: checking {hex(sid)}, is_member_id={idc.is_member_id(sid)}")
        if sid in processed_members:
            logging.debug(f"post_func_name_change: Skipping {hex(sid)} - already processed")
            continue
        ...
        if idx == -1:
            logging.debug(f"post_func_name_change: Could not get member info for {hex(sid)} (is_member_id={idc.is_member_id(sid)})")
            continue
        
        vtable_struct_name = vtable_tif.get_type_name()
        logging.debug(f"post_func_name_change: Processing member {hex(sid)} from vtable struct {vtable_struct_name}")
        ...
        if not class_name:
            logging.debug(f"post_func_name_change: No class name extracted, processing member directly")
            if _add_vtable_member_rename(vtable_tif, idx, udm.name, new_field_name,
                                         args_list, processed_classes, vtable_tif.get_tid(),
                                         member_offset):
                logging.debug(f"post_func_name_change: Added member to args_list (now {len(args_list)} members)")
        ...
        logging.debug(f"post_func_name_change: Processing original member for class {class_name}")
        if _add_vtable_member_rename(vtable_tif, idx, udm.name, new_field_name,
...
def funcname2fieldname(name, field_offset=None):
    ...
    if "::" in name: # not demangled name may have '::' ?
        name = name.split("::")[-1]
    if name.startswith("~") or " destructor" in name: # space is intentional
        name = "dtor"
...
def update_vtable_struct(
    ...
    while func is not None:
        ...
        new_func_name, is_renamed = update_func_name_with_class(func, class_name, overwrite=overwrite, func_name=new_func_name)

        if new_field_name is None:
            vtable_offset = function_count * utils.WORD_LEN  # Offset in bytes
            new_field_name = funcname2fieldname(new_func_name, vtable_offset)

        func_ptr = None
        if is_renamed and ida_hexrays.init_hexrays_plugin():
            ...
            # Parse function pointer declaration properly using idc_parse_decl
            func_ptr_decl = f"int (__thiscall *)({class_name} *this)"
            til = ida_typeinf.get_idati()
            parsed = ida_typeinf.idc_parse_decl(til, func_ptr_decl + ";", 1)
            if parsed:
                func_ptr = ida_typeinf.tinfo_t()
                if not func_ptr.deserialize(til, parsed[1], None, None):
...
def make_vtable(
    ...
    if isinstance(struct_size, str) and parent_name is None:
        parent_name = struct_size
        struct_size = None

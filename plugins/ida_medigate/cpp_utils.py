...
AUTOGENERATED_FUNC_NAME_RE = re.compile(r"^(null)?sub_[A-F0-9]+$")
...
def can_rename_func_name(name):
    return not name.startswith("nullsub_") and not name.startswith("ret_")
...
def update_func_name_with_class(func_ea, class_name, overwrite=False, func_name=None):
    ...
    if not can_rename_func_name(name):
        return name, False
...
def post_func_name_change(new_name, ea):
    ...
    if is_autogenerated_func_name(new_field_name):
    ...
    for vtable_tif, member_tid, member_offset, field_name, class_name in renamed_member_info:
        ...
        for func_ea in func_eas:
            ...
            # Skip if function name cannot be renamed (e.g., nullsub_)
            if not can_rename_func_name(current_name):
                continue
...
def post_struct_member_name_change(udt_name, udm, new_name):
    ...
    # Don't trigger renames if the new method name is autogenerated
    if is_autogenerated_func_name(new_name):
